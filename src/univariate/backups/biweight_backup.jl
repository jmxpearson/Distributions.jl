...
#### Conversions
function convert{T <: Real, S <: Real}(::Type{Biweight{T}}, μ::S, σ::S)
    Biweight(T(μ), T(σ))
end
function convert{T <: Real, S <: Real}(::Type{Biweight{T}}, d::Biweight{S})
    Biweight(T(d.μ), T(d.σ))
end
